<template>
  ...
  <main class=" font-barlow text-white text-center grid grid-cols-6  
      place-content-center   pt-3 max-w-sm md:max-w-xl mt-12 md:mt-24">
      ...
      <div class=" relative">
        <div v-if="playerWon" class=" duration-150 ease-in transition-all">
          <p class=" absolute h-44 w-44 top-7 -left-16 
            md:h-64 md:w-64 md:-top-8 md:-left-24 rounded-full bg-Neutral-DarkText opacity-25 z-10  "> </p>
          <p class=" absolute h-52 w-52 -left-20 top-2 
            md:h-80 md:w-80 md:-top-16 md:-left-32 rounded-full bg-Neutral-DarkText opacity-25 z-10 "> </p>
          <p class=" absolute h-64 w-64 -left-24 -top-4
            md:h-128 md:w-128  md:-top-28 md:-left-44 rounded-full bg-Neutral-DarkText opacity-25 z-10  "></p>
        </div>

        <div :class="pickedImage.containerClass" class="mt-16	 z-20">
          <img :src="pickedImage.src" alt="" class="relative z-30 " :key="pickedImage.key">
        </div>
      </div>
      <p class="text-lg tracking-wider w-32 -ml-7 text-left justify-left grid z-20 ">YOU PICKED</p>
    ...
    <div class=" grid gap-y-24 grid-cols-1 col-start-5 md:row-start-1 md:col-start-6  relative ">
      <div>
        <div v-if="houseWon">
          ...
          md:h-64 md:w-64 md:-top-7 md:-right-16 rounded-full md:bg-red-500 sm:bg-Neutral-DarkText opacity-25 z-10">
          </p>
          <p class=" absolute h-52 w-52 -right-20 -top-2
          md:h-80 md:w-80 md:-top-14 md:-right-24 rounded-full bg-Neutral-DarkText opacity-25 z-10"> </p>
          <p class=" absolute h-64 w-64 -right-24 -top-8
          md:h-128 md:w-128 md:-top-24 md:-right-36 rounded-full bg-Neutral-DarkText opacity-25 z-10"> </p>
        </div>
        <div>
          <div v-if="showShadow"
          ...
          <component :is="houseChoice" class="before:right-9 mt-16 z-20"></component>


        </div>
      </div>
      <p class="text-lg tracking-wider -ml-20 w-52 z-20">THE HOUSE PICKED</p>

    </div>
    <div v-if="picks"
      class="grid place-content-center row-start-2 col-start-2 md:col-start-2 col-span-4 md:row-start-1 gap-y-4 font-barlow mt-20">
      ...
      <button @click="handlePlayAgain" :style="{ color: textColor }" class="uppercase font-barlow	text-Neutral-DarkText bg-white w-40 m-auto py-2 
        rounded-md text-lg border-2 border-white z-40 tracking-wildest ">
        play again
      </button>
    </div>
  </main>
